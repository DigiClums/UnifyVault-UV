...
export class PaymasterProvider implements IPaymasterProvider {
  ...
  async getPaymasterStubData(
    ...
    return {
      ...
      // Deliberately omit preVerificationGas / verificationGasLimit / callGasLimit.
      // Returning them here would short-circuit viem's `prepareUserOperation`, which
      // only calls `eth_estimateUserOperationGas` when those fields are undefined.
      // Hardcoding verificationGasLimit=150000 starves first-deployment initCode
      // (factory.createAccount ≈ 176,769 gas) and yields AA13 initCode/OOG.
    };
  }
