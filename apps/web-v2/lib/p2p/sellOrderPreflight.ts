...
export function computeSellOrderPreflight(params: PreflightBalanceParams): PreflightBalanceResult {
  ...
  const symbol = validatedSymbol || assetVal.assetInfo?.symbol || getTokenSymbol(asset);
  ...
  if (side === 'SELL') {
    if (!userAddress) {
      ...
      errorMessage =
        assetVal.errorMessage || 'Selected asset is not supported on the active network.';
  ...
  const contractValid =
    isNative || (isContractDeployed !== false && !contractReadError && validatedDecimals !== null);
