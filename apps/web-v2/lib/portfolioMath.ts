...
/**
 * Aggregates the connected EOA and its deterministically-derived ERC-4337 Smart Account
 * into a single user-controlled portfolio for dashboard accounting.
 *
 * The two addresses are separate on-chain holders, but together they represent one
 * user-controlled portfolio. A pure wallet-to-wallet transfer between them only
 * redistributes balances; the aggregated totals remain invariant (no new basis, no P&L).
 */
export interface PortfolioAddressBalances {
  eoaSharesRaw: bigint;
  smartAccountSharesRaw: bigint;
  eoaCostBasisRaw: bigint;
  smartAccountCostBasisRaw: bigint;
}

export function aggregatePortfolioAddresses(balances: PortfolioAddressBalances): {
  totalSharesRaw: bigint;
  totalCostBasisRaw: bigint;
} {
  return {
    totalSharesRaw: balances.eoaSharesRaw + balances.smartAccountSharesRaw,
    totalCostBasisRaw: balances.eoaCostBasisRaw + balances.smartAccountCostBasisRaw,
  };
}
