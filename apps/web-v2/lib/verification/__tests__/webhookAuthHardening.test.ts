...
describe('Phase 7.2.7 — M2 Webhook Authentication Hardening Suite', () => {
  ...
  beforeEach(async () => {
    ...
    if (fs.existsSync(vRoot)) try { fs.rmSync(vRoot, { recursive: true }); } catch {}
    if (fs.existsSync(iRoot)) try { fs.rmSync(iRoot, { recursive: true }); } catch {}
  ...
  it('1. Rejects unknown or untrusted provider with HTTP 403 Forbidden', async () => {
    const req = new NextRequest('http://localhost:3000/api/p2p/payment-verification/webhook/unknown_provider', {
      method: 'POST',
  ...
  it('2. Rejects webhook requests missing signature or timestamp headers with HTTP 401', async () => {
    const req = new NextRequest('http://localhost:3000/api/p2p/payment-verification/webhook/bank_webhook_provider', {
      method: 'POST',
      body: JSON.stringify({ tradeId: mockTradeId, providerReference: 'TX123' }),
    });
  ...
  it('6. Replay Protection: Rejects duplicate providerReference consumption', async () => {
    ...
    const req1 = new NextRequest('http://localhost:3000/api/p2p/payment-verification/webhook/bank_webhook_provider', {
      method: 'POST',
      headers: {
        'x-webhook-signature': validSignature,
        'x-webhook-timestamp': timestamp,
    ...
    const res1 = await POST(req1, { params: Promise.resolve({ provider: 'bank_webhook_provider' }) });
    expect(res1.status).toBe(200);
    const data1 = await res1.json();
    expect(data1.verificationResult.status).toBe('VERIFIED');

    // Replay attempt with identical providerReference -> REJECTED
    const req2 = new NextRequest('http://localhost:3000/api/p2p/payment-verification/webhook/bank_webhook_provider', {
      method: 'POST',
      headers: {
        'x-webhook-signature': validSignature,
        'x-webhook-timestamp': timestamp,
    ...
    const res2 = await POST(req2, { params: Promise.resolve({ provider: 'bank_webhook_provider' }) });
