...
describe('FreshBaseSepolia Deployment Sequence', () => {
  it('should contain exactly 53 sequential transactions', () => {
    expect(FRESH_BASE_SEPOLIA_DEPLOYMENT_STEPS).toHaveLength(53);
    FRESH_BASE_SEPOLIA_DEPLOYMENT_STEPS.forEach((step, idx) => {
      expect(step.stepNumber).toBe(idx + 1);
    });
  });

  it('should have 15 DEPLOY steps and 38 CALL steps', () => {
    ...
    expect(deploySteps).toHaveLength(15);
    expect(callSteps).toHaveLength(38);
  });
  ...
  it('should have artifacts for all 15 deployed contracts', () => {
