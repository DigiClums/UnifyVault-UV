    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-card border border-border-subtle backdrop-blur-xl">
        ...
        <div className="flex items-center space-x-2 shrink-0">
          <div className="flex items-center space-x-1 bg-muted p-1 rounded-lg border border-border-subtle">
            <button
      ...
      {/* View Mode Switcher: Grid View ONLY vs Table View ONLY */}
      {viewMode === 'grid' ? (
          ...
          action={
            <div className="flex items-center space-x-1 bg-surface p-1 rounded-xl border border-border-subtle">
              <button
                onClick={() => setViewMode('grid')}
                aria-label="Grid view"
                className="p-1.5 rounded-lg text-xs transition-colors text-muted-foreground hover:text-foreground"
              >
                <LayoutGrid className="w-3.5 h-3.5" />
              </button>
              <button
                onClick={() => setViewMode('table')}
                aria-label="Table view"
                className="p-1.5 rounded-lg text-xs transition-colors bg-[#BFFF00] text-black shadow-[2px_2px_0_#000]"
              >
                <List className="w-3.5 h-3.5" />
              </button>
            </div>
              ...
              {isLoading ? (
                ...
                activeHoldings.map((asset) => (
                  <tr key={asset.symbol} className="hover:bg-card/40 transition-colors">
                    ...
                    <td
                      className="py-3 px-3 text-foreground font-mono"
                      title={`${asset.balanceFormatted} ${asset.symbol}`}
                      aria-label={`Exact Balance: ${asset.balanceFormatted} ${asset.symbol}`}
                    >
                      {formatDisplayCryptoBalance(asset.balanceFormatted, asset.symbol)}
                    </td>
