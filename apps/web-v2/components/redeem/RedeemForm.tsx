...
export function RedeemForm() {
  ...
  const eoaSharesBalNum = parseFloat(formatUnits(sharesBalance, 18)) || 0;
  const eoaSharesBalFormatted = formatShares(sharesBalance);
  ...
  const activeBalanceNum = source === 'gasless' ? saSharesBalNum : eoaSharesBalNum;
  const activeBalanceFormatted =
    source === 'gasless' ? saSharesBalFormatted : eoaSharesBalFormatted;

  const handleSourceSelect = (nextSource: 'eoa' | 'gasless') => {
      ...
      <Card className="space-y-4 relative overflow-hidden bg-card border-2 border-black dark:border-white/15 shadow-glass p-4 sm:p-5 rounded-2xl">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => handleSourceSelect('eoa')}
            ...
            <button
              type="button"
              onClick={() => handleSourceSelect('gasless')}
              className={`p-3 rounded-xl border text-left transition-all ${
                source === 'gasless'
                  ? 'bg-[#BFFF00]/10 border-[#BFFF00] text-foreground'
                  : 'bg-muted/40 border-border-subtle text-muted-foreground hover:border-border'
          ...
          {source === 'gasless' && (
            <div className="flex items-center justify-between gap-2">
              ...
              {isGaslessSupported ? (
                ...
                <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-500 text-[10px] font-bold uppercase tracking-wider shrink-0">
                  Gasless unavailable
                </span>
              )}
            </div>
        ...
        <div className="space-y-2.5">
          <div className="flex justify-between items-center text-xs text-slate-600 dark:text-slate-400">
            ...
            <span className="font-mono">
              Available:{' '}
              <span className="font-bold text-foreground">{activeBalanceFormatted} UVBE</span>
            </span>
          </div>
        ...
        {!isConnected ? (
          ...
          >
            {isRedeeming ? (
              ...
              <span>Redeem Gasless & Receive USDC</span>
            ) : (
