...
export interface PaymasterConfirmationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  title: string;
  description: string;
  actionLabel: string;
  actionColor?: 'purple' | 'rose' | 'amber' | 'emerald' | 'blue';
  isPending?: boolean;
  details?: { label: string; value: string | React.ReactNode }[];
  warningMessage?: string;
}

export function PaymasterConfirmationModal({
  isOpen,
  onClose,
  onConfirm,
  title,
  ...
  details = [],
  warningMessage,
}: PaymasterConfirmationModalProps) {
  ...
  const colorStyles = {
    ...
    blue: 'bg-blue-600 hover:bg-blue-500 text-white focus:ring-blue-500/50',
  }[actionColor];
        ...
        {warningMessage && (
          <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-start space-x-2.5">
            ...
            <div className="space-y-0.5">
              <p className="font-bold text-amber-200">Preflight Notice</p>
