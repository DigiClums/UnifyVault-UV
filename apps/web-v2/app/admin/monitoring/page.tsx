...
export default function AdminMonitoringPage() {
  ...
  const btcFresh = (contractReads?.[0]?.result as boolean) ?? false;
  const ethFresh = (contractReads?.[1]?.result as boolean) ?? false;
  const btcPriceRaw = (contractReads?.[2]?.result as bigint) || 0n;
  const ethPriceRaw = (contractReads?.[3]?.result as bigint) || 0n;
  ...
  const [indexerHealth, setIndexerHealth] = useState<'Healthy' | 'Error'>('Healthy');
  const [indexerLastBlock, setIndexerLastBlock] = useState<number>(0);

  useEffect(() => {
    async function checkIndexer() {
      try {
        const res = await fetch('http://localhost:3006/api/indexer/stats');
        if (res.ok) {
          const data = await res.json();
          setIndexerHealth('Healthy');
          setIndexerLastBlock(data.lastBlock || 0);
        } else {
    ...
    checkIndexer();
    const interval = setInterval(checkIndexer, 10_000);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-border-subtle/50">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <StatusBadge
              status={
                isOracleHealthy && indexerHealth === 'Healthy' && !isReadError ? 'Healthy' : 'Error'
              }
              label={
                isOracleHealthy && indexerHealth === 'Healthy' && !isReadError
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value="Base Sepolia"
          subtitle={`Chain ID 84532 | Block ${blockNumber ? blockNumber.toString() : '...'}`}
          icon={Layers}
          glowColor="blue"
        />
        <StatCard
        ...
        <StatCard
          ...
          value={indexerHealth.toUpperCase()}
          subtitle={`Block Sync ${indexerLastBlock}`}
          icon={Database}
          glowColor={indexerHealth === 'Healthy' ? 'cyan' : 'amber'}
        />
      </div>
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-border-subtle/40 font-mono">
            ...
            <tr className="hover:bg-card/40 transition-colors">
              ...
              <td className="py-3.5 px-3 text-slate-400">Every 10s</td>
              <td className="py-3.5 px-3 font-sans">
                <StatusBadge status={indexerHealth} label={indexerHealth.toUpperCase()} />
              </td>
              <td className="py-3.5 px-3 text-right font-sans text-emerald-400 font-semibold">
                Block {indexerLastBlock}
              </td>
            </tr>
