  ...
  try {
    ...
    if (!isTestMode && !isAuthorizedAdmin(caller)) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized: Only authorized admin can execute dispute resolution actions.' },
        { status: 403 },
      );
    }
    ...
    if (adminAction === 'MARK_BUYER_FAVOURED') {
      ...
      disputeRecord.resolutionNotes = resolutionNotes || reason || 'Admin favored buyer after evidence review.';
    } else if (adminAction === 'MARK_SELLER_FAVOURED') {
      disputeRecord.status = 'CLOSED_SELLER_FAVORED';
      disputeRecord.resolvedAt = new Date().toISOString();
      disputeRecord.resolutionNotes = resolutionNotes || reason || 'Admin favored seller after evidence review.';
