...
export async function GET(req: NextRequest) {
  try {
    ...
    const action = searchParams.get('action') || req.headers.get('x-action') || 'dispute-chat-message';

    if (!tradeIdStr || isNaN(Number(tradeIdStr))) {
      return NextResponse.json({ success: false, error: 'Missing or invalid tradeId.' }, { status: 400 });
    }
    const tradeId = Number(tradeIdStr);

    if (!userAddress || !isAddress(userAddress)) {
      return NextResponse.json({ success: false, error: 'Missing or invalid userAddress.' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ success: false, error: `Trade #${tradeId} not found.` }, { status: 404 });
    }
    ...
    if (!isBuyer && !isSeller && !isAdmin) {
      return NextResponse.json(
        {
          success: false,
          error: 'Unauthorized: Private dispute chat is restricted to trade participants and authorized admins.',
        },
        { status: 403 },
      );
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { tradeId, userAddress, signature, timestamp, action, content, evidenceHash, evidenceUrl } = body;
    ...
    if (!isAuthBypassedForTest) {
      if (!signature || !timestamp) {
        return NextResponse.json({ success: false, error: 'Signature and timestamp required.' }, { status: 401 });
      }
    ...
    if (!isBuyer && !isSeller && !isAdmin) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized: Only trade participants or admins can post dispute messages.' },
        { status: 403 },
      );
    }
