...
| :-------------------------------- | :--------: | :------: | :---------------------------------------------------------------------------------------------------------------------------------- | :-----------: | :----------: |
| **1. Donation Attack**            |    Low     |   Low    | Vault NAV updates dynamically; direct token transfers increase share value pro-rata for all holders without minting unbacked shares |    **Low**    | **VERIFIED** |
| **2. First Depositor Inflation**  |    Low     |   High   | Genesis `$1.00/share` baseline fallback; zero-value deposits revert explicitly with `Errors.InvalidDepositAmount()`                 |    **Low**    | **VERIFIED** |
...
| **4. Oracle Failure / Staleness** |   Medium   |   High   | Enforces heartbeat staleness checks, rejects 0/negative prices, and routes via `try...catch` to fallback feeds                      |  **Medium**   | **VERIFIED** |
...
- **Attack Scenario**: The first depositor attempts to deposit 0 or 1 wei of asset, receive 1 wei of shares, then donate 100 WETH to inflate the share price.
- **Impact**: Subsequent small depositors lose collateral due to integer rounding to 0 shares.
- **Protocol Mitigation**: `UnifyVaultController.sol` explicitly **reverts zero-value deposits** with `Errors.InvalidDepositAmount()`. `calculateSharePriceUSD()` defaults to `$1.00/share` when share supply is `0n`, making inflation attacks unviable.
- **Residual Risk**: **Low** (Zero-value deposits are rejected on-chain).
...
- **Attack Scenario**: A Chainlink oracle feed freezes, returns a stale price, or reverts due to a network glitch.
- **Impact**: Vault collateral drain via stale or corrupted pricing.
- **Protocol Mitigation**: `OracleManager.sol` validates heartbeat freshness: `block.timestamp - rawRound.updatedAt <= config.heartbeat`. If the primary provider reverts or lags past heartbeat, `OracleManager` wraps the call in `try...catch` and routes to the secondary `fallbackProvider`. If both fail, transactions revert with `Errors.AssetNotSupported()`.
- **Residual Risk**: **Medium** (Dependent on external Chainlink/Pyth oracle infrastructure availability).
