 ...
 * @notice Upgrade-safe UUPS implementation of UnifyVaultController behind an ERC1967 Proxy
 ...
 */
contract UnifyVaultControllerUpgradeable is
  Initializable,
  ...
  UUPSUpgradeable
{
  ...
  /**
   * @notice Initializes the UUPS upgradeable controller instance
   * @param admin_ The initial administrative address granted admin, governance, guardian, and bot roles
   * @param directory_ The ProtocolDirectory contract address
   * @param oracle_ The OracleManager contract address
   * @param vault_ The CustodyVault contract address
   * @param treasury_ The Treasury contract address
   * @param token_ The UVBTCETHToken index share contract address
   */
  function initialize(
    address admin_,
    address directory_,
    ...
    _maxDepositPerTx = type(uint256).max;
    _maxRedeemPerTx = type(uint256).max;
    _dailyDepositCap = type(uint256).max;
    _dailyRedeemCap = type(uint256).max;

    _largeDepositThreshold = 10_000 * 1e6; // $10,000 default (USDC 6 decimals)
    _largeRedeemThreshold = 10_000 * 1e18; // 10,000 shares default (18 decimals)
    _swapSlippageBps = 100; // 1% default

    _grantRole(DEFAULT_ADMIN_ROLE, admin_);
    _grantRole(AccessRoles.GOVERNANCE_ROLE, admin_);
    _grantRole(GUARDIAN_ROLE, admin_);
    _grantRole(BOT_ROLE, admin_);
  }

  /**
   * @dev UUPS upgrade authorization hook restricted strictly to GOVERNANCE_ROLE
   */
