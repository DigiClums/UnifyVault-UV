...
contract UnifyVaultUpgradeableTest is Test {
  ...
  function testModuleCannotRollbackVersion() public {
    MockModuleV1 moduleV1 = new MockModuleV1();
    bytes32 id = moduleV1.moduleId();
    ...
    vm.prank(governance);
    vm.expectRevert(UnifyVaultUpgradeable.ModuleVersionNotIncreasing.selector);
    vault.upgradeModule(address(moduleV1));

    (address implementationAddress, uint64 version, bool enabled) = vault.getModule(id);
    assertEq(implementationAddress, address(moduleV1));
    assertEq(version, 1);
    assertTrue(enabled);
  }
