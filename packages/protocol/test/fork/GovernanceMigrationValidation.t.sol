  ...
  function test_GovernanceMigrationAndTimelockOwnership() public {
    ...
    assertTrue(
      timelock.hasRole(
        ...
        address(0xd905920c91853039060246Ed5724AA72B91a96DA)
      )
    );
    assertTrue(
      timelock.hasRole(
    ...
    for (uint256 i = 0; i < targets.length; i++) {
      IAccessControl ac = IAccessControl(targets[i]);
      assertTrue(
        ac.hasRole(DEFAULT_ADMIN_ROLE, newAdmin),
        'New Admin must hold DEFAULT_ADMIN_ROLE'
      );
    ...
    for (uint256 i = 0; i < targets.length; i++) {
      IAccessControl ac = IAccessControl(targets[i]);
      assertFalse(
        ac.hasRole(DEFAULT_ADMIN_ROLE, oldAdmin),
        'Old Admin must NOT hold DEFAULT_ADMIN_ROLE'
      );
    ...
    console.log(
      '[PASS] Task 6: Governance Migration verified - Deployer completely revoked, Timelock in sole control'
    );
  }
}
