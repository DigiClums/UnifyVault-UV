...
/**
 * @title PhaseHStabilizerVaultForkTest
 * @notice Comprehensive Phase H Dynamic Liquidity-Aware Fork Integration Test Suite for StabilizerVault on Base Mainnet (Chain ID 8453)
 */
contract PhaseHStabilizerVaultForkTest is Test {
  ...
  // --- Test 3: Normal Liquidity -> Max Allowed Dynamic Trade ($100 Ceiled) ---
  function test_03_normalLiquidityMaxDynamicTrade() public {
    uint256 nav = vault.getAuthoritativeNAV();
    uint256 dexPrice = (nav * 99) / 100; // 1% below NAV (100 BPS deviation)

    // Normal pool liquidity = $100,000 USDC depth
    vm.prank(admin);
    ...
    assertTrue(shouldExecute, 'Should execute stabilization under normal liquidity');
    assertTrue(isBuy, 'Direction should be BUY');
    assertEq(devBps, 100, 'Deviation should be 100 BPS');
    // In $100,000 liquidity, 50 BPS impact trade = $1000, ceiled to HARD_MAX_TRADE_USDC = 100 USDC
    assertEq(tradeAmount, 100 * 1e6, 'Trade amount should be ceiled at 100 USDC');
    assertEq(reason, 'READY_TO_STABILIZE');
  }

  // --- Test 4: Shallow Liquidity -> Trade Size Dynamically Scaled Down ---
  function test_04_shallowLiquidityDynamicDownsizing() public {
    uint256 nav = vault.getAuthoritativeNAV();
    uint256 dexPrice = (nav * 995) / 1000; // 50 BPS deviation

    // Shallow pool liquidity = $1,250 USDC depth
    // Safe trade size at 50 BPS impact: $1,250 * 50 * 2 / 10000 = $12.50 USDC
    vm.prank(admin);
    vault.setMockMarketState(true, dexPrice, 1_250 * 1e6);

    (bool shouldExecute, bool isBuy, , uint256 tradeAmount, string memory reason) = vault
      .checkStabilization();

    assertTrue(shouldExecute, 'Should execute shallow trade within safe limits');
    assertTrue(isBuy, 'Direction should be BUY');
    assertEq(tradeAmount, 12_500_000, 'Trade amount dynamically scaled down to exactly 12.50 USDC');
    assertEq(reason, 'READY_TO_STABILIZE');
    ...
    assertTrue(newDex > dexPrice, 'DEX price must converge upward toward NAV safely');
  }

  // --- Test 5: Very Low Liquidity -> NO TRADE (Blocked to Prevent Unsafe Price Impact) ---
  function test_05_veryLowLiquidityNoTrade() public {
    uint256 nav = vault.getAuthoritativeNAV();
    uint256 dexPrice = (nav * 99) / 100; // 100 BPS deviation

    // Very low pool liquidity = $50 USDC depth (below $100 minPoolLiquidity threshold)
    vm.prank(admin);
    vault.setMockMarketState(true, dexPrice, 50 * 1e6);

    (bool shouldExecute, , , uint256 tradeAmount, string memory reason) = vault
      .checkStabilization();

    assertFalse(shouldExecute, 'Trade MUST be blocked in very low liquidity');
    assertEq(tradeAmount, 0, 'Trade size must be zero');
    assertEq(reason, 'INSUFFICIENT_SAFE_LIQUIDITY');
  }

  // --- Test 6: DEX Above NAV -> Dynamically Scaled SELL Execution ---
  function test_06_dexAboveNavDynamicSell() public {
    uint256 nav = vault.getAuthoritativeNAV();
    uint256 dexPrice = (nav * 101) / 100; // 1% above NAV

    // Pool liquidity = $2,500 USDC depth -> safe trade = $25.00 USDC
    vm.prank(admin);
    vault.setMockMarketState(true, dexPrice, 2_500 * 1e6);
    ...
    assertEq(tradeAmount, 25 * 1e6, 'Trade dynamically sized at 25 USDC');
    assertEq(reason, 'READY_TO_STABILIZE');
  ...
  // --- Test 7: Extreme Deviation (>= 200 BPS) -> Emergency Halt ---
  function test_07_emergencyHaltOnExtremeDeviation() public {
    uint256 nav = vault.getAuthoritativeNAV();
  ...
  // --- Test 8: Cooldown Enforced (300 seconds) ---
  function test_08_cooldownEnforcement() public {
    uint256 nav = vault.getAuthoritativeNAV();
    ...
    // Re-assert price below NAV
    vm.prank(admin);
    vault.setMockMarketState(true, dexPrice, 100_000 * 1e6);

    // 2nd Trade immediately after blocked
  ...
  // --- Test 10: Governance Hard Bound Invariants ---
  function test_10_governanceParameterHardBounds() public {
    vm.startPrank(admin);
  ...
  // --- Test 11: Keeper Privilege Boundaries ---
  function test_11_keeperPrivilegeBoundaries() public {
  ...
  // --- Test 12: Governance Non-Arbitrary Inventory Withdrawal ---
  function test_12_governanceInventoryWithdrawal() public {
  ...
  // --- Test 13: Existing Protocol State Invariants Unchanged ---
  function test_13_existingProtocolInvariantsUnchanged() public {
