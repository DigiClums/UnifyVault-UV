...
contract BaseSepoliaV2MigrationTest is Test {
  ...
  function setUp() public {
    ...
    v2Escrow = new P2PEscrowV2(TREASURY, 100, address(v2CBM)); // 1% fee
  ...
  function test_ForkMigration_V1TokenRejection() public {
    ...
    // Creating a P2P trade with V1 Token must NOT affect V2 CostBasisManager
    vm.startPrank(SELLER);
    ...
    assertEq(v2CBM.escrowTradeBasis(tradeId), 0);
  }
  ...
  function test_ForkMigration_V2P2PEscrowFullLifecycleAndReplay() public {
    ...
    // Verify basis moved to escrow trade basis during funding
    assertEq(v2Token.balanceOf(address(v2Escrow)), tradeShares);
    assertGt(v2CBM.escrowTradeBasis(tradeId), 0);
    assertEq(v2CBM.realizedPnL(SELLER), sellerPnlBefore); // 0 PnL realization during fund

    uint256 tradeBasis = v2CBM.escrowTradeBasis(tradeId);
    ...
    // Verify Seller PnL realized exactly once
    int256 expectedGain = int256(fiatAmountUSD) - int256(tradeBasis);
    assertEq(v2CBM.realizedPnL(SELLER), sellerPnlBefore + expectedGain);

    // Verify Buyer receives net shares & $200 USD acquisition basis
    uint256 feeShares = (tradeShares * 100) / 10000; // 1%
    assertEq(v2Token.balanceOf(BUYER), EXP_BUYER_BAL + (tradeShares - feeShares));
    assertEq(v2CBM.costBasis(BUYER), BUYER_COST_BASIS + fiatAmountUSD);
