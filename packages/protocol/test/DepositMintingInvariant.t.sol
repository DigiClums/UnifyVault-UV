...
import './DepositMinting.t.sol'; // imports MockERC20 and ITestTreasury

contract DepositMintingHandler {
  ...
  uint256 public cumulativeExpectedShares = 0;
  ...
  function deposit(uint256 amount) public {
    ...
    try controller.deposit(address(tokenA), amount, 0, user) {
      ...
      cumulativeExpectedShares += expectedShares;
    } catch {}
    vm.stopPrank();
  }
}

contract DepositMintingInvariantTest is Test {
  UnifyVaultController public controller;
  CustodyVault public vault;
  ITestTreasury public treasury;
  UVBTCETHToken public token;
  DepositMintingHandler public handler;
  ...
  function setUp() public {
    ...
    handler = new DepositMintingHandler(controller, vault, treasury, token, tokenA);
  ...
  // Invariant 3: Treasury contains exactly the cumulative successfully deposited protocol fees (excluded from vault)
  ...
  // Invariant 4: Share supply matches cumulative expected shares exactly
  function invariant_shareConservation() public {
    assertEq(token.totalSupply(), handler.cumulativeExpectedShares());
  }
}
