...
contract ExecutePhase74RegressionScript is Script {
  ...
  function run() external {
    ...
    try escrow.getTrade(3) returns (EscrowTypes.Trade memory t) {
      t3Pre = t;
      console2.log('   Trade #3 State:', uint8(t.state));
    } catch {}
    try escrow.getTrade(4) returns (EscrowTypes.Trade memory t) {
      t4Pre = t;
      console2.log('   Trade #4 State:', uint8(t.state));
    } catch {}
    try escrow.getTrade(5) returns (EscrowTypes.Trade memory t) {
      t5Pre = t;
      console2.log('   Trade #5 State:', uint8(t.state));
    } catch {}
    try escrow.getTrade(6) returns (EscrowTypes.Trade memory t) {
      t6Pre = t;
      console2.log('   Trade #6 State:', uint8(t.state));
    } catch {}
    ...
    if (t3Pre.tradeId > 0) {
      ...
      require(
        t.seller == t3Pre.seller &&
          t.buyer == t3Pre.buyer &&
          t.amount == t3Pre.amount &&
          t.state == t3Pre.state,
        'Trade #3 altered'
      );
    ...
    if (t4Pre.tradeId > 0) {
      ...
      require(
        t.seller == t4Pre.seller &&
          t.buyer == t4Pre.buyer &&
          t.amount == t4Pre.amount &&
          t.state == t4Pre.state,
        'Trade #4 altered'
      );
    ...
    if (t5Pre.tradeId > 0) {
      ...
      require(
        t.seller == t5Pre.seller &&
          t.buyer == t5Pre.buyer &&
          t.amount == t5Pre.amount &&
          t.state == t5Pre.state,
        'Trade #5 altered'
      );
    ...
    if (t6Pre.tradeId > 0) {
      ...
      require(
        t.seller == t6Pre.seller &&
          t.buyer == t6Pre.buyer &&
          t.amount == t6Pre.amount &&
          t.state == t6Pre.state,
        'Trade #6 altered'
      );
